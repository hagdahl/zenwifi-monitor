...
12. Run `python scripts/check_versions.py` to confirm that every tracked file carries the current project version. It prints a per-file summary and exits 0 when green, 1 on drift and 2 when `VERSION` or Git is unavailable. Add `--json` for a machine-readable result.
13. If a run fails before SQLite opens, inspect `%LOCALAPPDATA%\ZenWiFiMonitor\bootstrap-errors.log`. The silent VBS wrapper does not display standard error output.
